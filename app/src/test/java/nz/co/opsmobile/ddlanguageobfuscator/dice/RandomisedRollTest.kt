...
class RandomisedRollTest{

    @Test
    fun `test that roll result contains 1 die`() {
        val result = RandomisedRoll(D6).result()
    ...
    fun `test that roll result contains rolled value`() {
        val expected = 3
        val result = RandomisedRoll(D6, randomiser = TestRandom(expected)).result()
    ...
    fun `test that value of one die is returned`() {
        val expected = 3
        val result = RandomisedRoll(D6, randomiser = TestRandom(expected, 5)).result()
        assertThat(result.rolls[0].rolled).isEqualTo(expected)
    }
    ...
    fun `test multiple dice`() {
        val expected = 4
        val result = RandomisedRoll(D12, expected).result()
    ...
    fun `test total of two rolls sum to total`() {
        ...
        val actual = RandomisedRoll(D6, 2, randomiser = TestRandom(roll1, roll2)).result()
    ...
    fun `test that modifiers are applied`() {
        ...
        val actual = RandomisedRoll(D6, 2, modifier, randomiser = TestRandom(roll1, roll2)).result()
        assertThat(actual.total).isEqualTo(expected)
    }
